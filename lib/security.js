...
export const NCR_N = 8;
export const NCR_Q = 257;
...
export function requireSameOrigin(req, res) {
  ...
  // Некоторые браузерные запросы могут не содержать Origin.
  ...
  /*
   * Некоторые GET-запросы могут не иметь Origin.
   */
  if (!origin) {
    return true;
  }
...
/*
 * Проверка матрицы NCR-LWE.
 *
 * В нашем текущем прототипе:
 * N = 8
 * Q = 257
 *
 * Каждая матрица должна быть 8x8.
 * Значения хранятся по модулю Q.
 */
export function isValidNcrMatrix(matrix) {
  if (!Array.isArray(matrix) || matrix.length !== NCR_N) {
    return false;
  }

  for (const row of matrix) {
    if (!Array.isArray(row) || row.length !== NCR_N) {
      return false;
    }
...
export function isValidPublicKey(publicKey) {
  return Boolean(
    ...
    isValidNcrMatrix(publicKey.A) &&
    isValidNcrMatrix(publicKey.B)
  );
}

export function isValidCiphertext(ciphertext) {
  return Boolean(
    ciphertext &&
    typeof ciphertext === 'object' &&
    isValidNcrMatrix(ciphertext.U) &&
    isValidNcrMatrix(ciphertext.V)
  );
}

export function isValidByteArray(value, exactLength = null) {
